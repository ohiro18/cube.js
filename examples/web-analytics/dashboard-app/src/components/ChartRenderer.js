...
const getType = (resultSet, key) => {
  let result = resultSet.serialize().loadResponse.results[0];

  if (result.annotation.measures[key]) {
    return result.annotation.measures[key].type;
  }

  if (result.annotation.dimensions[key]) {
    return result.annotation.dimensions[key].type;
  }
...
const TypeToChartComponent = {
  ...
  number: ({ resultSet, height }) => {
    ...
    const format = resultSet.serialize().loadResponse.results[0].annotation.measures[measureKey].format;
